...
CitySchema.methods = {
  ...
  getViewInOSRMLink: function(city_to) {
    return (
      ...
      this.getLon() +
      ";" +
      city_to.loc.coordinates[1] +
      "," +
      city_to.loc.coordinates[0]
    );
  },
  routeTo: function(city_to, callback) {
    ...
    client.get(query_str, function(error, response, body) {
      if (!error && response.statusCode == 200) {
        ...
        logger.error(
          "Houve um erro ao buscar a rota entre " +
            self.fullName() +
            " e " +
            city_to.fullName()
        );
    ...
    self.routeTo(targetCity, function(err, routeAB) {
      ...
      targetCity.routeTo(self, function(err, routeBA) {
        ...
        route = {
          id: targetCity,
          distance: distance,
          routeForwardDistanceRatio:
            routeABDistance > 0 ? (routeABDistance / distance - 1) * 100 : 0,
          routeBackwardDistanceRatio:
            routeABDistance > 0 ? (routeBADistance / distance - 1) * 100 : 0
        };
        ...
        if (route.routeForwardDistanceRatio >= 50) {
          ...
          route.routeForwardDistanceRatio > 0 &&
          route.routeForwardDistanceRatio < 50
        )
        ...
        if (route.routeBackwardDistanceRatio >= 50) {
          ...
          route.routeBackwardDistanceRatio > 0 &&
          route.routeBackwardDistanceRatio < 50
        ) {
          self.stats.totalConnected += 1;
        } else {
          self.stats.totalInexistent += 1;
    ...
    self.save(function(err) {
      if (err) {
        ...
        self.findNearest(cities_qty, function(err, nearCities) {
          ...
          async.eachSeries(
            nearCities,
            function(nearCity, doneCheckingAConnection) {
              self.checkConnectionTo(
                nearCity.obj,
                nearCity.dis,
                doneCheckingAConnection
              );
            },
            function(err) {
              if (err) logger.error(err);
              ...
              self.stats.percentualConnected =
                self.stats.totalConnected / self.stats.totalChecked || 0;
              ...
              self.save(function(err) {
                if (err) logger.error(err);
                  ...
                  logger.info(
                    "A atualização de " + self.fullName() + " terminou."
                  );
              });
            }
          );
  ...
  findNearestPromise: function(limit) {
    var City = mongoose.model("City");

    return this.model("City").collection.geoNear(
      this.getLon(),
      this.getLat(),
      { spherical: true, num: limit + 1, distanceMultiplier: 6371 })
      .then(cities => {
  ...
  findNearest: function(limit, callback) {
    ...
    this.model("City").collection.geoNear(
      this.getLon(),
      this.getLat(),
      { spherical: true, num: limit + 1, distanceMultiplier: 6371 },
      function(err, cities) {
...
CitySchema.statics = {
  load: function(id, doneLoading) {
    this.findOne({ _id: id }).exec(doneLoading);
  },
    ...
    csv()
      ...
      .on("record", function(row, index) {
        City.findOne({ ibge_id: row.ibge_id }, function(err, city) {
          ...
          city.loc = {
            type: "Point",
            coordinates: [new Number(row.lon), new Number(row.lat)]
          };
          city.save();
        });
      })
